...
- `apps/hosted-checkout`：Hosted Checkout 收银台应用，Vue 3 + Tailwind CSS。
...
兼容说明：`dev:cashier` 和 `build:cashier` 暂时保留为别名，实际会转发到 Hosted Checkout 的新命令。
...
- `hosted-checkout` 默认启动在 `http://localhost:5175`，预留 `/checkout` 到 `http://127.0.0.1:8000`。
