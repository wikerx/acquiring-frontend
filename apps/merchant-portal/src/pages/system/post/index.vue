<template>
    <div class="page system-page">
        ...
        <el-table v-loading="loading" :data="filteredRows" row-key="postId" size="small">
            ...
            <el-table-column label="操作" width="180" align="center"><template #default="{ row }"><el-button v-if="canEdit" link type="primary" @click="openForm(row)">编辑</el-button><el-button v-if="canDelete" link type="danger" @click="remove(row)">删除</el-button><span v-if="!canEdit && !canDelete">-</span></template></el-table-column>
        </el-table>
...
async function loadData() {
    loading.value = true;
    try { rows.value = await systemApi.posts(); } finally { loading.value = false; }
}
function applyQuery() {}
function resetQuery() { query.keyword = ''; query.status = undefined; }
