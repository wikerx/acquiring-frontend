<template>
    <div class="app-container">
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5"><el-button type="primary" plain :icon="Plus" size="small" @click="handleAdd()" v-hasPermi="'system:menu:add'">{{ $t('common.add') }}</el-button></el-col>
        ...
        <StandardTable table-key="system-menu" v-if="refreshTable" v-loading="loading" :data="rows" row-key="menuId" :default-expand-all="isExpandAll" :tree-props="{ children: 'children' }" size="small">
...
const { Search, Refresh, Plus, Edit, View, Sort } = Icons;
...
import { createMenu, treeMenus, updateMenu, updateMenuStatus, type SysMenu } from '@/api/system/menu';
...
onMounted(()=>loadData());
async function loadData(){loading.value=true;try{const r=await treeMenus({menuName:tv(query.menuName),menuType:tv(query.menuType),status:nv(query.status)});rows.value=r.map(nr);total.value=cn(rows.value)}catch{rows.value=[];total.value=0}finally{loading.value=false}}
