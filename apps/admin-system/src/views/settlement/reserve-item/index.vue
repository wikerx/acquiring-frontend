...
<template>
    <div class="app-container settlement-list-page">
        <el-form v-show="showSearch" :model="query" inline size="small" class="search-form" label-width="92px">
            <el-form-item :label="t('transaction.settlement.reserveActionNo')"><el-input v-model.trim="query.reserveActionNo" clearable @keyup.enter="handleSearch" /></el-form-item>
            ...
            <el-form-item :label="t('transaction.settlement.reserveStatus')"><el-select v-model="query.reserveStatus" clearable filterable><el-option v-for="value in reserveStatuses" :key="value" :label="enumText('reserveStatusValue', value)" :value="value" /></el-select></el-form-item>
            ...
            <el-form-item :label="t('transaction.settlement.expectedReleaseDate')"><el-date-picker v-model="expectedReleaseDateRange" type="daterange" value-format="YYYY-MM-DD" clearable :range-separator="t('common.to')" :start-placeholder="t('common.startTime')" :end-placeholder="t('common.endTime')" /></el-form-item>
        ...
        <el-drawer v-model="detailVisible" :title="t('transaction.settlement.reserveDetail')" size="min(760px, 96vw)" destroy-on-close>
            <template v-if="detail">
                ...
                <el-alert class="reserve-detail__status-help" type="info" :closable="false" show-icon :title="enumText('reserveStatusDescriptionValue', detail.reserveStatus)" />
                ...
                <el-descriptions :column="2" border size="small" class="reserve-detail__descriptions">
                    ...
                    <el-descriptions-item :label="t('transaction.fields.merchantId')"><el-button link type="primary" @click="openMerchant(detail.merchantId)">{{ detail.merchantId }}</el-button></el-descriptions-item><el-descriptions-item :label="t('transaction.settlement.fundAccount')"><el-button link type="primary" @click="openAccount(detail)">{{ fundAccountText(detail) }}</el-button></el-descriptions-item>
                    <el-descriptions-item :label="t('transaction.settlement.merchantOrderNo')">{{ detail.merchantOrderNo || '-' }}</el-descriptions-item><el-descriptions-item :label="t('transaction.fields.transactionId')"><el-button v-if="detail.sourceTransactionId" link type="primary" @click="openTransaction(detail)">{{ detail.sourceTransactionId }}</el-button><span v-else>-</span></el-descriptions-item><el-descriptions-item :label="t('transaction.settlement.batchNo')" :span="2"><el-button v-if="detail.settlementBatchNo" link type="primary" @click="openBatch(detail.settlementBatchNo)">{{ detail.settlementBatchNo }}</el-button><span v-else>-</span></el-descriptions-item>
...
const transactionTimeRange=ref<[string,string]|[]>([]), expectedReleaseDateRange=ref<[string,string]|[]>([]);
...
onMounted(async()=>{query.settlementBatchNo=routeText('settlementBatchNo');query.reserveNo=routeText('reserveNo');query.sourceTransactionId=routeText('sourceTransactionId');const linkedBusinessDate=businessDateFromBusinessNo(query.settlementBatchNo);if(linkedBusinessDate)dateRange.value=[linkedBusinessDate,linkedBusinessDate];await loadData();if(query.reserveNo){const linkedRow=rows.value.find((row)=>row.reserveNo===query.reserveNo);if(linkedRow)openDetail(linkedRow)}});
    function requestQuery(): SettlementReserveItemQuery { return {...query, settlementBatchNo:query.settlementBatchNo||undefined, merchantId:query.merchantId||undefined, reserveNo:query.reserveNo||undefined, reserveActionNo:query.reserveActionNo||undefined, sourceTransactionId:query.sourceTransactionId||undefined, merchantOrderNo:query.merchantOrderNo||undefined, reserveStatus:query.reserveStatus||undefined, actionType:query.actionType||undefined, currency:query.currency||undefined, beginTransactionTime:transactionTimeRange.value[0]||undefined, endTransactionTime:transactionTimeRange.value[1]||undefined, beginExpectedReleaseDate:expectedReleaseDateRange.value[0]||undefined, endExpectedReleaseDate:expectedReleaseDateRange.value[1]||undefined, beginBusinessDate:dateRange.value[0], endBusinessDate:dateRange.value[1]}; }
...
function handleSearch(){query.pageNo=1;loadData()}
    function handleReset(){Object.assign(query,{settlementBatchNo:undefined,merchantId:undefined,reserveNo:undefined,reserveActionNo:undefined,sourceTransactionId:undefined,merchantOrderNo:undefined,reserveStatus:undefined,actionType:undefined,currency:undefined,pageNo:1});dateRange.value=defaultDateRange();transactionTimeRange.value=[];expectedReleaseDateRange.value=[];loadData()}
...
function openAccount(row:SettlementReserveItem){router.push({path:'/fund/account',query:{merchantId:row.merchantId,accountNo:row.accountNo||undefined}})}
...
function openBatch(settlementBatchNo:string){router.push({path:'/settlement/reserve-candidates',query:{view:'batches',settlementBatchNo}})}
function fundAccountText(row:SettlementReserveItem){return row.accountNo||`#${row.accountId}`}
...
<style scoped>
.search-form :deep(.el-input),.search-form :deep(.el-select){width:220px}.search-form :deep(.el-date-editor){width:330px}.reserve-detail__identity{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;border-left:4px solid #0f766e;padding:10px 14px;background:#f4f9f8}.reserve-detail__status-help{margin-bottom:16px}.reserve-detail__flow{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin-bottom:18px;border:1px solid var(--el-border-color-lighter)}.reserve-detail__flow>div{padding:13px;border-right:1px solid var(--el-border-color-lighter)}.reserve-detail__flow>div:last-child{border-right:0}.reserve-detail__flow span{display:block;color:var(--el-text-color-secondary);font-size:12px}.reserve-detail__flow strong{display:block;margin-top:5px}.reserve-detail__flow .is-current{background:#eef8f5}.reserve-detail__descriptions :deep(.el-descriptions__content){text-align:center;overflow-wrap:anywhere}@media(max-width:640px){.reserve-detail__flow{grid-template-columns:1fr}.reserve-detail__flow>div{border-right:0;border-bottom:1px solid var(--el-border-color-lighter)}}
</style>
