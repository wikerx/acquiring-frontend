...
export const layoutPreferenceStorage = {
    sidebarWidthKey(userKey?: string | number | null) {
        return `vexra:${APP_CODE}:layout:sidebar-width:${safeUserKey(userKey)}`;
    },
    readSidebarWidth(userKey: string | number | null | undefined, defaultWidth: number, minWidth: number, maxWidth: number) {
        const raw = localStorage.getItem(this.sidebarWidthKey(userKey));
    ...
    writeSidebarWidth(userKey: string | number | null | undefined, width: number) {
        try {
            localStorage.setItem(this.sidebarWidthKey(userKey), String(width));
        } catch {
            // UI preference persistence should never block page rendering.
        }
    },
